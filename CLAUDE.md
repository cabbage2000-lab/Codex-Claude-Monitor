...
- `src/extension.js`: VS Code entry point and the only file that depends on the `vscode` API. It owns the main status bar item (priority 90; click runs `agentTokenStatus.refresh`), the refresh timer with default 10s interval, configuration reads for `agentTokenStatus.sessionsRoot`, `claudeRoot`, and `refreshIntervalMs`, and the severity-to-theme-color mapping via `SEVERITY_COLORS` (`charts.green` / `charts.yellow` / `charts.red`). It also owns a second status bar item `handoffItem` (priority 89, `$(export) 交接`) shown only when `contextPercent >= HANDOFF_THRESHOLD` (80); clicking it runs `agentTokenStatus.handoff`, which composes a handoff prompt via `src/handoff.js`, writes it to the clipboard, and shows an information message. Surfacing the suffix is the only automatic behavior — generating and copying the prompt is strictly user-triggered. The latest `usage` is cached in `latestUsage` so the command does not need to re-read JSONL.
- `src/handoff.js`: pure logic layer for the handoff feature, decoupled from `vscode`. `buildHandoffPrompt(usage, gitInfo)` composes a paste-ready continuation prompt: a header instruction, an auto-collected metadata block (provider, sessionFile, context tokens/window/percent via the `format*` helpers from `agentUsage`, Claude token composition, git branch/status/recent commits), then the promptify-style English section skeleton (`Context to read first` / `Progress so far` / `Where it stopped` / `Next` / `Constraints` / `Done when` / `Stop if`) whose placeholders are filled by the current session's Claude after paste. Missing fields are omitted so the prompt degrades gracefully. `collectGitInfo(cwd)` runs `git` through `child_process` and returns `{ branch, status, recentCommits }` or `null` when `cwd` is not a repo.
