...
- `src/claudeUsage.js`: recursively scans `~/.claude/projects` for `.jsonl`, picks the newest file, and reads the last `type: "assistant"` entry with `message.usage`. Context tokens = `input_tokens + cache_read_input_tokens + cache_creation_input_tokens`; the context window is inferred from the model name by `inferClaudeContextWindow`: model names containing `1m` or `claude-opus-4-7` / `claude-opus-4-8` use 1M, otherwise 200k. File selection skips files without an assistant usage entry (probe sessions, freshly started sessions), trying the newest 10 by mtime.
